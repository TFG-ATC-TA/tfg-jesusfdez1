...
export default function AccelerometerChart({ bucket }: { bucket: string }){
  ...
  const connectWebSocket = () => {
    ...
    wsRef.current.onmessage = (event) => {
      try {
        ...
        if (!rawData.payload || !Array.isArray(rawData.payload)) {
          ...
          timestamp: Math.floor(item.timestamp), // Asegurar timestamp entero
        ...
        setData(prevData => {
          const combinedData = [...prevData, ...newDataPoints]
            ...
            .slice(-1000) // Mantener solo los últimos 1000 puntos
          return combinedData
        })
        setLoading(false)
      } catch (err) {
    ...
    wsRef.current.onclose = (event) => {
      ...
      // Si el código es 1008, es un token inválido
      if (event.code === 1008) {
  ...
  useEffect(() => {
    if (chartContainerRef.current) {
      ...
      const accelXSeries = chart.addLineSeries({
        color: isDarkMode ? 'rgba(239, 68, 68, 0.8)' : 'rgba(185, 28, 28, 0.8)', 
        lineWidth: 1,
        title: 'Eje X',
      })
      const accelYSeries = chart.addLineSeries({ 
        color: isDarkMode ? 'rgba(34, 197, 94, 0.8)' : 'rgba(21, 128, 61, 0.8)', 
        lineWidth: 1,
        title: 'Eje Y',
      })
      const accelZSeries = chart.addLineSeries({ 
      ...
      const formatData = (data: AccelData[], key: 'accel_x' | 'accel_y' | 'accel_z'): LineData[] => {
        return data
          .map(point => ({
            ...
            value: point[key],
          }))          .sort((a, b) => (a.time as number) - (b.time as number)) // Asegurar orden ascendente
      }
      ...
      accelXSeries.setData(formatData(sortedData, 'accel_x'))
      accelYSeries.setData(formatData(sortedData, 'accel_y'))
      accelZSeries.setData(formatData(sortedData, 'accel_z'))
  ...
  return (
    <Card className="w-full">      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="flex-1 min-w-0">
            ...
            <CardDescription className="mt-1">
              Lecturas en tiempo real del acelerómetro (m/s²)
            </CardDescription>
          </div>          <Badge 
      ...
      </CardHeader><CardContent className="px-4 sm:px-6">        {error ? (
