...
const TemperatureGyroscopeLive: React.FC<TemperatureGyroscopeLive> = ({ data, theme, systemTheme }) => {
  ...
  useEffect(() => {
    ...
    seriesRef.current.gyroY = chart.addLineSeries({
      color: isDarkMode ? "rgba(34, 197, 94, 0.8)" : "rgba(21, 128, 61, 0.8)",
      lineWidth: 1,
      title: "Eje Y",
    })

    seriesRef.current.gyroZ = chart.addLineSeries({
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (sortedData.length > 0) {
        seriesRef.current.gyroX?.setData(formatData(sortedData, "gyro_x"))
        seriesRef.current.gyroY?.setData(formatData(sortedData, "gyro_y"))
        seriesRef.current.gyroZ?.setData(formatData(sortedData, "gyro_z"))
      }
    }, 0)
  }, [data])
    ...
    wsRef.current.onmessage = (event) => {
      try {
        ...
        const newDataPoints = rawData.payload.map((item: {
          ...
          gyro_x: item.fields.gyro_x,
          gyro_y: item.fields.gyro_y,
          gyro_z: item.fields.gyro_z,
        }))
      ...
      <CardContent className="px-4 sm:px-6">
        {error ? (
          ...
          <ChartContainer className="h-[300px] sm:h-[400px] w-full" config={{}}>
            {isClient ? (
              <TemperatureGyroscopeLive data={data} theme={theme} systemTheme={systemTheme} />
            ) : (
