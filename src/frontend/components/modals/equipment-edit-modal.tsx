...
export default function EquipmentEditModal({
  ...
  const getDeviceTypesByEquipmentType = (equipmentType: string) => {
    switch (equipmentType) {
      case "Tanque de leche":
        return ["Monitor de tanque"];
      case "Estación de lavado":
        return ["Monitor de estación de lavado"];
      case "Monitor de leche":
        return ["Monitor de leche"];
      default:
        return ["Monitor de tanque", "Monitor de estación de lavado", "Monitor de leche"];
    }
  };

  const [selectedFilters, setSelectedFilters] = useState<Record<string, string[]>>({
    type: getDeviceTypesByEquipmentType(''),
  });
  ...
  // Inicializar filtros cuando se abre el modal
  useEffect(() => {
    if (isOpen) {
  ...
  // Modificar fetchDevices para usar isFetchingRef y mejorar el manejo de estados
  ...
  const fetchTanks = useCallback(async (farmId: string, page: number = 1, searchTerm: string = '') => {
  ...
  useEffect(() => {
    if (isOpen && equipmentId) {
      const initializeData = async () => {
        try {
          setInitialLoading(true);
          await fetchDevices(equipmentInfo.farm, 1, '');
        } catch (error) {
  ...
  }, [isOpen, equipmentId, equipmentInfo.farm, fetchDevices]);

  // Asegurar que los datos no se borren al cambiar filtros o paginación
  useEffect(() => {
    if (isOpen && equipmentInfo.farm) {
      fetchDevices(equipmentInfo.farm, devicesPage, devicesSearchTerm);
    }
  }, [devicesPage, devicesSearchTerm, isOpen, equipmentInfo.farm, fetchDevices]);

  // Modificar fetchEquipment para cargar los datos del equipamiento seleccionado
  useEffect(() => {
    if (isOpen && equipmentId) {
      const fetchEquipment = async () => {
        ...
        try {
          ...
          console.log("Datos del equipamiento:", data);
          ...
          // Pre-seleccionar los dispositivos asociados
          const deviceSelections: Record<string, boolean> = {};
          if (data.device && Array.isArray(data.device)) {
            data.device.forEach((device: any) => {
              deviceSelections[device._id] = true;
            });
          }
          setSelectedDevices(deviceSelections);

          // Pre-seleccionar los tanques asociados si es una estación de lavado
          const tankSelections: Record<string, boolean> = {};
          if (data.associatedTanks && Array.isArray(data.associatedTanks)) {
            data.associatedTanks.forEach((tank: any) => {
              tankSelections[tank._id] = true;
            });
          }
          setSelectedTanks(tankSelections);

          // Actualizar filtros basados en el tipo
          const deviceTypes = getDeviceTypesByEquipmentType(data.type);
          setSelectedFilters({ type: deviceTypes });

          // Cargar dispositivos y tanques una vez que tenemos el farmId
          if (data.farm) {
            await Promise.all([
              fetchDevices(data.farm, 1, ''),
              data.type === "Estación de lavado" ? fetchTanks(data.farm, 1, '') : Promise.resolve()
            ]);
          }
  ...
  }, [isOpen, equipmentId, session, fetchDevices, fetchTanks]);

  // Manejar cambios de paginación y búsqueda para tanques
  useEffect(() => {
    if (isOpen && equipmentInfo.farm && equipmentInfo.type === "Estación de lavado" && !initialLoading) {
      fetchTanks(equipmentInfo.farm, tanksPage, tanksSearchTerm);
    }
  }, [tanksPage, tanksSearchTerm, isOpen, equipmentInfo.farm, equipmentInfo.type, fetchTanks, initialLoading]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({
        description: "Equipamiento actualizado con éxito",
        variant: "success",
      });
      onClose();
      onRefresh();
            ...
            <Card>
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <Button id="submit-equipment-button" type="submit" disabled={!isFormValid || loading}>
                    {loading ? "Actualizando..." : "Actualizar equipamiento"}
                  </Button>
                </form>
              </CardContent>
            </Card>
